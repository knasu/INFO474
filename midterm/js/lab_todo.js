...
(function() {
  ...
  function drawTicks(limits) {
    ...
    // + turns into a number (would get string otherwise)
    // returns gre row 
    ...
    // domain: x values (gre scores)
    // range: area on svg where domain will map to
    // returns a FUNCTION that can be used to scale the gre score to an x coordinate on the svg
    let xScale = d3.scaleLinear()
      .domain([limits.greMin, limits.greMax])
      .range([50, 450]) // margin of 50 on both sides of x axis
    ...
    svgContainer.append('g')
      .attr('transform', 'translate(0, 450)') // centered in svg
      .call(xAxis); // continues chain and callsa  function
    ...
    let yScale = d3.scaleLinear()
      .domain([limits.admitMax, limits.admitMin-0.05]) // to invert values
      .range([50, 450])
